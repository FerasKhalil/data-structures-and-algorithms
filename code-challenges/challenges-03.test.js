...
Write a function called addTwo that takes in an array and adds two to every value using a 
for loop. Place the new value in a new array. Return the new array.
...
const oddValues = (arr) => {
  // Solution code here...
  let newArr=[];
  arr.filter(checkElement=>{
    if(checkElement%2!==0)
    {
      newArr.push(checkElement);
    }
  });
  return newArr;
};
...
Write a function named notInFirstArray that, given two arrays as input, uses filter to
 return an array of all the elements in the second array that are not included in the first array.
